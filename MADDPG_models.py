...
class MADDPG(object):
    ...
    def learn(self):
        ...
        with torch.no_grad():  # target_Q has no gradient
            ...
            target_Q_n = []
            for i in range(self.agent_num):
                target_Q = batch_r[:,i].reshape((-1,1)) + self.GAMMA * self.critic_target_n[i](batch_s2_, next_action)
                target_Q_n.append(target_Q)

        # Get the current Q
        for i in range(self.agent_num):
            current_Q = self.critic_n[i](batch_s2, batch_a)
            # Compute the critic loss
            critic_loss = F.mse_loss(current_Q, target_Q_n[i])
            print(critic_loss)
            # Optimize the critic
        ...
        # Freeze critic networks so you don't waste computational effort
            ...
            # Compute actor loss
            actor_loss = -self.critic_n[i](batch_s2, batch_a_copy).mean()  # Only use Q1
            # Optimize the actor
